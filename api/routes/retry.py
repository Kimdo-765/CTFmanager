...
def _retry_preamble(prev_id: str, hint: str) -> str:
    ...
    return (
        ...
        f"tree. The partial exploit.py / solver.py / report.md / "
        f"scratch the previous agent wrote sit directly at `./` — run "
        f"`ls` (NOT `ls ./work/` — that's a doubled path and will "
        f"fail) only if you need to confirm. If your conversation "
        f"context already shows the prior reasoning + tool history, "
        f"just continue from where you left off in light of the hint "
        f"below. If you don't see prior context (rare — SDK couldn't "
        f"locate the prior session), `ls` and read whichever file "
        f"matters before applying the hint.\n\n"
        f"{hint}"
    )
...
def _resume_preamble(prev_id: str, hint: str) -> str:
    ...
    return (
        ...
        f"Your prior session was halted mid-run. Your current working "
        f"directory IS the prior attempt's work tree — whatever files "
        f"you had already written are sitting directly at `./`. If "
        f"your conversation context still has all the prior reasoning "
        f"+ tool history, continue exactly where you left off and "
        f"apply the new guidance below — do not restart the analysis. "
        f"If you don't see prior context (rare — SDK couldn't locate "
        f"the prior session), `ls` (NOT `ls ./work/` — that's a "
        f"doubled path) and read whichever file matters before "
        f"applying the hint.\n\n"
        f"{hint}"
    )
